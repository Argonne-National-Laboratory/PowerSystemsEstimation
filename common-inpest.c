...
/* Generator constants */
const PetscScalar H0[3]   = {23.64,6.4,3.01};   /* Inertia constant (const declare)*/
      PetscScalar H[3]    = {23.64,6.4,3.01};   /* Inertia constant (used as global variable)*/
...
const PetscScalar PD0_ref[3]     = {1.25, 0.9, 1.0};  /* const declare corresponding to steady state */
      PetscScalar PD0_disturb[3] = {5.5, 0.9, 1.0};  /* const declare corresponding to disturbed state */
      PetscScalar PD0[3]         = {1.25, 0.9, 1.0};  /* global variable used in the DAE */
...
PetscErrorCode ResidualFunction(SNES snes,Vec X, Vec F, Userctx *user)
{
  ...
  */
  ...
  for (i=0; i < ngen; i++) {
    ...
    //printf("-- %g\n", (-TM[i] + Edp*Id + Eqp*Iq + (Xqp[i] - Xdp[i])*Id*Iq)/M[i]);
...
#undef __FUNCT__
#define __FUNCT__ "ModelSetup"
PetscErrorCode ModelSetup(Userctx *user)
{
  PetscErrorCode     ierr;
  PetscViewer        Ybusview, Xview;
  PetscInt           *idx2, i;
  ...
  ierr = PetscOptionsBegin(PETSC_COMM_WORLD,NULL,"Transient stability fault options","");CHKERRQ(ierr);
  {
    ...
    user->tdisturb = 0.1;
    ierr           = PetscOptionsReal("-tdisturb","","",user->tdisturb,&user->tdisturb,NULL);CHKERRQ(ierr);
    user->trestore = 0.3;
    ierr           = PetscOptionsReal("-trestore","","",user->trestore,&user->trestore,NULL);CHKERRQ(ierr);
    user->tfinal   = 5.0;
    ierr           = PetscOptionsReal("-tfinal","","",user->tfinal,&user->tfinal,NULL);CHKERRQ(ierr);

    user->dt       = 0.01; //0.01;
    ierr           = PetscOptionsReal("-dt","","",user->dt,&user->dt,NULL);CHKERRQ(ierr);

    user->data_dt  = 0.01; //0.02;
    ierr           = PetscOptionsReal("-data_dt","","",user->data_dt,&user->data_dt,NULL);CHKERRQ(ierr);
    ...
    user->saveSol  = PETSC_TRUE;
    ierr           = PetscOptionsBool("-saveSol","","",user->saveSol,&user->saveSol,NULL);CHKERRQ(ierr);

    user->data_noise = 0.01;
    ierr            = PetscOptionsReal("-data_noise","","",user->data_noise,&user->data_noise,NULL);CHKERRQ(ierr);

    user->prior_noise = 0.1;
    ierr            = PetscOptionsReal("-prior_noise","","",user->prior_noise,&user->prior_noise,NULL);CHKERRQ(ierr);

    user->outputCov = PETSC_FALSE;
    ierr            = PetscOptionsBool("-outputCov","","",user->outputCov,&user->outputCov,NULL);CHKERRQ(ierr);

    user->loadObsFile[0] = '\0';
    ierr = PetscOptionsString("-loadObservations", "Specify file with observations", "", user->loadObsFile, user->loadObsFile, 1024, NULL);

    //PD0_disturb[0]  = 4.25;
    ierr            = PetscOptionsReal("-load_disturb","","",PD0_disturb[0],PD0_disturb,NULL);CHKERRQ(ierr);
...
PetscErrorCode ForwardSolve(Vec P,void *ctx0, PetscErrorCode (*PostStepFunction)(TS))
{
  ...
  H[0] = P_array[0]; H[1] = P_array[1]; H[2] = P_array[2];
 ...
 /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  ierr = IJacobian(ts,ctx->t0,X,Xdot,0.0,ctx->J,ctx->J,ctx);CHKERRQ(ierr);
  ...
  /*   ierr = TSSolve(ts,X);CHKERRQ(ierr); */
  /* } else { */
  /*   printf("Ignoring trestore since tfinal <= trestore.\n"); */
  /* } */
